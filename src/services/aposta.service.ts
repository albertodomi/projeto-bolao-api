...
export class ApostaService {
  static async create(userId: number, data: z.infer<typeof CreateApostaSchema>) {
    ...
    if (opcao.campanha.status !== "ABERTA") {
      throw new Error("Campanha não está aberta para apostas")
    }
    const now = new Date()
    if (now < opcao.campanha.dtInicio || now > opcao.campanha.dtFim) {
      throw new Error("A campanha não está aceitando apostas por conta do período")
    }
